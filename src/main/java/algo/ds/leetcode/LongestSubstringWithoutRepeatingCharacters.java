...
public class LongestSubstringWithoutRepeatingCharacters {
	 ...
	 * duplicate character. But it is unnecessary. If a substring s_{ij}s ij ​ from
	 * index ii to j - 1j−1 is already checked to have no duplicate characters. We
	 * only need to check if s[j]s[j] is already in the substring s_{ij}s ij ​ .
	 * 
	 * To check if a character is already in the substring, we can scan the
	 * substring, which leads to an O(n^2)O(n 2 ) algorithm. But we can do better.
	 * 
